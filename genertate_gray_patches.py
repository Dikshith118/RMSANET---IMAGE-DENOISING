...
def generate_patches(img_dir, patch_size, num_patches_per_image, save_path):
    """
    Generates and saves image patches from a directory of full images.
    ...
    if os.path.exists(original_img_dir):
        image_files = [os.path.join(original_img_dir, f) for f in os.listdir(original_img_dir) if f.endswith(('.png', '.jpg', '.bmp'))]
    else:
        image_files = [os.path.join(img_dir, f) for f in os.listdir(img_dir) if f.endswith(('.png', '.jpg', '.bmp'))]
    ...
    all_patches_np = np.stack(all_patches, axis=0)

    # Add a channel dimension for consistency
    all_patches_np = np.expand_dims(all_patches_np, axis=-1)
...
if __name__ == '__main__':
    ...
    PATCH_SIZE = 50
    NUM_PATCHES_PER_IMAGE = 500  # Number of patches to generate from each image
    TRAIN_DATA_DIR = os.path.join('data', 'BSD432')
    SAVE_FOLDER_PATH = os.path.join('data','trained_patches')
    SAVE_FILE_PATH = os.path.join(SAVE_FOLDER_PATH, 'train_patches.npy')
